...
void TDriver::getModelAcceleration()
{
    ...
    float ang = RtTrackSideTgAngleL(&(oCar->_trkPos));

    float temp = ang;
    float angle_change = fabs(ang - last_angle);
    last_angle = temp;
    ...
    // float m2 = 0.4224066;
    // float m3 = -5.991554;
    // float model_acc = m1/(angle_change + m2) + m3;
// ===============================================================================
    // trained on snr
    // float m1 = 6.398284;
    // float m2 = 40.25413;
    // float m3 = 2.684326;
    // float m4 = -2.012542;
    // float model_acc = m1/(angle_change*m2 + m3) + m4;
    // oModelAcc = model_acc;
...
// ===============================================================================
    ...
    // float s2 = 0.02691791;
    // float s3 = 14.08976;
    // float desired_speed =  s1/(angle_change + s2) + s3;
    // printf("%f\n", desired_speed);
    ...
    if(tt == 2){
      ...
      float m1 = 0.5418887;
      float m2 = 0.01932527;
      float model_dacc = m1*(angle_change) + m2;
      oModelDAcc = model_dacc;
      if(oCar-> race.distFromStartLine < 400){
        oModelDAcc = 0.0;
      }
    }
...
*/
  ...
  if(initk == 0){
    ...
    if(tt == 1){
      ...
      CarBrakeCmd = (float) oBrake + 0.015;
    }
    if(tt == 2){
...
//======data logging================
  ...
  if(((int)tval_result.tv_usec) >= 100){
    ...
    data[counter][2] = (tdble)oCar->_gear;
    data[counter][3] = oCar->_brakeCmd;
    data[counter][4] = oCar->_clutchCmd;
...
//=========data logging==========
  ...
  sprintf(new_file_path,"%s%s.csv",dir,name);

  rename(tmp_file_path, new_file_path);

//=========data logging finishes here==========
