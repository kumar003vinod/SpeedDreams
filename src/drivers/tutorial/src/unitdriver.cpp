...
TV2D TDriver::getTargetPoint()
{
  tTrackSeg *seg = oCar->_trkPos.seg;
  float lookahead = oLookAhead + oCar->_speed_x*oLookAheadFactor;
  Tdble length = getDistToSegEnd();
  ...
  if ( seg->type == TR_STR) {
    ...
    return Add(s, Mult(length, d));
  } else {
    TV2D c;
    c.x = seg->center.x;
    c.y = seg->center.y;
    Tdble arc = length/seg->radius;
    const Tdble arcsign = (seg->type == TR_RGT) ? -1 : 1;
    arc = arc*arcsign;
    return Rot(c, c, arc);
  }
}
